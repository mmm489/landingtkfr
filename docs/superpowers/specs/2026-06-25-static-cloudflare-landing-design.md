...
- Keep `email=<email>` for compatibility with the original form.
...
- If the incoming URL already contains `email`, `sub4`, `password`, or `account`, the form values generated by this landing override those incoming values.
...
- Verify valid email builds the expected destination URL with every incoming query parameter, plus `email`, `sub4`, generated `password`, and `account`.
